...
export const GUIDES = [
  ...
  g('friendship', 'users',
    ...
    [
      ...
      b(t('The five stages, and what each one brings', 'ห้าระดับความสนิท และสิ่งที่ได้ในแต่ละระดับ'),
        t(`The game shows friendship as a gauge, not as a numbered level, and no official source publishes the underlying values. What it does describe is a fixed order of milestones, and every Pokémon walks the same five stages: (1) Just met — it keeps its distance and asks for things. (2) Warming up — it starts proposing games, quizzes about a friend of theirs and Look This Way. (3) Friendly — it looks pleased to see you and begins handing over materials it has found, without being asked. (4) Close — it drops the nickname and starts calling you by your given name instead of "Ditto"; the fifteen Pokémon with an emote to give hand it over around here. (5) Best Friends — it comes to you and says so, a Best Friend mark appears on its Pokédex entry, and it is always happy to see you.`,
          `เกมแสดงค่าความสนิทเป็นเกจ ไม่ใช่ตัวเลขระดับ และไม่มีแหล่งข้อมูลทางการที่เปิดเผยค่าจริงเบื้องหลัง สิ่งที่ระบุไว้ชัดคือลำดับเหตุการณ์ที่ตายตัว ซึ่งโปเกมอนทุกตัวจะไล่ผ่านห้าระดับเหมือนกันหมด คือ (1) เพิ่งรู้จัก มันจะยังอยู่ห่าง ๆ และเอาแต่ขอของ (2) เริ่มคุ้นเคย มันจะเริ่มชวนคุณเล่นเกม ทั้งควิซเกี่ยวกับเพื่อนของมันและ Look This Way (3) สนิทระดับหนึ่ง มันจะดูดีใจเวลาเจอคุณ และเริ่มเอาวัสดุที่มันหามาได้มาให้เองโดยไม่ต้องขอ (4) สนิทมาก มันจะเลิกเรียกคุณว่า "ดิตโต้" แล้วเปลี่ยนมาเรียกชื่อจริงของคุณ ส่วนโปเกมอน 15 ตัวที่มีอิโมตให้ ก็จะให้ในช่วงนี้ (5) เพื่อนซี้ มันจะเดินมาบอกคุณเอง มีเครื่องหมาย Best Friend ขึ้นในหน้าโปเกเด็กซ์ และจะดีใจที่ได้เจอคุณตลอดไป`),
        t(`Two honest caveats. The stage boundaries are not numbered on screen, so "stage 3" here means "the point at which the unprompted gifts start", not a value you can read anywhere. And no official source gives a rate for those unprompted gifts — how often a given Pokémon walks up with something is undocumented, so this wiki does not quote a number for it. The gifts that *are* pinned down — the Litter drops, the emotes, the once-a-day Dream Island run — are listed on the Gifts page.`,
          `มีสองข้อที่ต้องบอกตามตรง หนึ่งคือเส้นแบ่งระหว่างระดับไม่มีตัวเลขขึ้นบนจอ ดังนั้น "ระดับ 3" ในที่นี้หมายถึง "จุดที่มันเริ่มเอาของมาให้เอง" ไม่ใช่ค่าที่อ่านได้จากที่ไหน สองคือไม่มีแหล่งข้อมูลทางการที่ระบุความถี่ของการให้ของแบบนั้น ว่าโปเกมอนตัวหนึ่งจะเดินมาให้ของบ่อยแค่ไหนยังไม่มีใครบันทึกไว้ วิกินี้จึงไม่ระบุตัวเลขให้ ส่วนของขวัญที่ "ยืนยันได้" อย่างของที่ทิ้งไว้จากความถนัด Litter อิโมต และทริป Dream Island วันละครั้ง ดูได้ที่หน้าของขวัญ`)),
      b(t('What each Pokémon gives you', 'โปเกมอนแต่ละตัวให้อะไรเราบ้าง'),
        t(`Three channels are documented species by species, and they behave very differently. Litter is the reliable one: 41 Pokémon carry the Litter specialty and drop one fixed material near their home, again and again, for as long as they live there — Combee drops Honey, Jirachi drops Stardust, Shellder and Cloyster drop Seashells. It runs off the specialty, not off friendship, so it starts the day they move in. Emotes are the opposite: fifteen species hand over one specific emote, once, as a high-friendship gift, and that is the end of it. And a story character can give a fixed item — Chef Dente's three PP-restoring candies are the only case.`,
          `มีสามช่องทางที่มีบันทึกไว้เป็นรายตัว และทั้งสามทำงานต่างกันมาก ช่องทางที่พึ่งพาได้คือ Litter โปเกมอน 41 ตัวมีความถนัดนี้ และจะทิ้งวัสดุประจำตัวหนึ่งอย่างไว้แถวบ้านของมันซ้ำไปเรื่อย ๆ ตราบใดที่มันยังอยู่ตรงนั้น เช่น คอมบีทิ้งน้ำผึ้ง จิราจิทิ้งผงดาว เชลเดอร์กับคลอยสเตอร์ทิ้งเปลือกหอย ช่องทางนี้มาจากความถนัด ไม่ใช่ค่าความสนิท จึงเริ่มทำงานตั้งแต่วันแรกที่มันย้ายเข้ามา ส่วนอิโมตตรงกันข้าม มี 15 ตัวที่ให้อิโมตเฉพาะของมันหนึ่งอัน ให้ครั้งเดียวตอนสนิทมาก แล้วก็จบ และตัวละครในเนื้อเรื่องให้ไอเทมตายตัวได้ ซึ่งมีแค่กรณีเดียวคือลูกอมฟื้น PP สามชิ้นของเชฟเดนเต้`),
        t(`Everything else a Pokémon hands over is a specialty doing its job rather than a gift: Gather Honey trades special furniture for honey you bring, Collect turns Rainbow and Silver Feathers into rare goods at Gimmighoul and Gholdengo, Trade runs the scale at a cash register, and Rarify turns Star Pieces into Pokémetal. Those are exchanges — you put something in first.`,
          `สิ่งอื่น ๆ ที่โปเกมอนส่งให้คุณนับเป็นการทำงานของความถนัด ไม่ใช่ของขวัญ เช่น Gather Honey แลกเฟอร์นิเจอร์พิเศษกับน้ำผึ้งที่คุณเอาไปให้ Collect ให้กิมมิโกลกับโกลเดนโกเปลี่ยน Rainbow Feather กับ Silver Feather เป็นของหายาก Trade เปิดตาชั่งแลกของที่เครื่องคิดเงิน และ Rarify เปลี่ยน Star Piece เป็น Pokémetal ทั้งหมดนี้คือการแลก คุณต้องให้ของก่อน`)),
      b(t('Favourites', 'ของโปรด'),
        t(`Every item in the game belongs to several of 43 "favourite" categories — Round stuff, Shiny stuff, Wooden stuff, Ocean vibes, Spooky stuff and the rest. Each Pokémon likes exactly five of them, plus one of the five flavours and one of the six ambiences (its ideal habitat: Bright, Warm, Humid, Dry, Cool or Dark). Giving or placing matching items raises friendship and Comfy Level much faster, and you can spot it working: a matching item lays down with a bigger sparkle. Every Pokémon page lists its own seven, with the items in each category.`,
          `ไอเทมทุกชิ้นในเกมจะถูกจัดอยู่ในหมวด "ของโปรด" หลายหมวดจากทั้งหมด 43 หมวด เช่น ของกลม ของแวววาว ของไม้ บรรยากาศทะเล ของหลอน โปเกมอนแต่ละตัวจะชอบ 5 หมวดพอดี บวกกับรสอาหาร 1 จาก 5 รส และบรรยากาศ 1 จาก 6 แบบ (บรรยากาศที่ชอบ ได้แก่ สว่าง อบอุ่น ชื้น แห้ง เย็น มืด) การให้หรือวางของที่ตรงหมวดจะเพิ่มค่าความเป็นเพื่อนและ Comfy Level ได้เร็วกว่ามาก และสังเกตได้ง่าย ๆ คือของที่ตรงหมวดจะมีประกายใหญ่กว่าตอนวางลงพื้น ดูของโปรดทั้งเจ็ดอย่างพร้อมรายการไอเทมของแต่ละหมวดได้ในหน้าโปเกมอนแต่ละตัว`)),
